...
function parseGit(logArray) {
    'use strict';
    var GitLogInstances = [], hashRe = /\w{7}/, dateRe = /\w+, \d{1,2} \w+ \d{4} \d{2}:\d{2}:\d{2} -\d{4}/, msgRe = /"(.*)"/, hash, date, msg, i;

    for (i = 0; i < logArray.length; i += 1) {
        hash = hashRe.exec(logArray[i]);
        date = dateRe.exec(logArray[i]);
        date = new Date(date);
        msg = msgRe.exec(logArray[i])[1];
